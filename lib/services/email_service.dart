import 'dart:convert';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;

class EmailService extends ChangeNotifier {
  final String _baseUrl = 'https://api.jarvis.cx/api/v1';
  final String _authToken = 'eyJhbGciOiJFUzI1NiIsImtpZCI6InFsWUdfYXNMRTI0VSJ9..oqYM5aMMiuF-Cg9RpcbmvAEw9a3SRpckKr2NyxQ58aMM-yBRzR9y3ogaeMJHzeXtVNtacuFsMd04roDlGGtwKQ';
  final String _guidHeader = 'baf60c1e-c61b-496d-ad92-f5aeeadf4def';
  
  bool _isLoading = false;
  String _error = '';
  int _remainingUsage = 50; // Default value
  
  bool get isLoading => _isLoading;
  String get error => _error;
  int get remainingUsage => _remainingUsage;
  
  // Generate email response based on action type
  Future<String> generateEmailResponse({
    required String emailContent,
    required String mainIdea,
    required String action,
    required String subject,
    required String sender,
    required String receiver,
    String language = 'vietnamese',
  }) async {
    try {
      _isLoading = true;
      notifyListeners();
      
      final response = await http.post(
        Uri.parse('$_baseUrl/ai-email'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $_authToken',
          'x-jarvis-guid': _guidHeader,
        },
        body: jsonEncode({
          'mainIdea': mainIdea,
          'action': action,
          'email': emailContent,
          'metadata': {
            'context': [],
            'subject': subject,
            'sender': sender,
            'receiver': receiver,
            'style': {
              'length': 'long',
              'formality': 'neutral',
              'tone': 'friendly'
            },
            'language': language
          }
        }),
      );
      
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        _remainingUsage = data['remainingUsage'] ?? _remainingUsage;
        _isLoading = false;
        notifyListeners();
        return data['email'] ?? '';
      } else {
        throw Exception('Failed to generate email: ${response.statusCode}');
      }
    } catch (e) {
      _error = e.toString();
      _isLoading = false;
      notifyListeners();
      rethrow;
    }
  }
  
  // Get reply ideas for an email
  Future<List<String>> getReplyIdeas({
    required String emailContent,
    required String subject,
    required String sender,
    required String receiver,
    String language = 'vietnamese',
  }) async {
    try {
      _isLoading = true;
      notifyListeners();
      
      final response = await http.post(
        Uri.parse('$_baseUrl/ai-email/reply-ideas'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $_authToken',
          'x-jarvis-guid': _guidHeader,
        },
        body: jsonEncode({
          'action': 'Suggest 3 ideas for this email',
          'email': emailContent,
          'metadata': {
            'context': [],
            'subject': subject,
            'sender': sender,
            'receiver': receiver,
            'language': language
          }
        }),
      );
      
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        _isLoading = false;
        notifyListeners();
        return List<String>.from(data['ideas'] ?? []);
      } else {
        throw Exception('Failed to get reply ideas: ${response.statusCode}');
      }
    } catch (e) {
      _error = e.toString();
      _isLoading = false;
      notifyListeners();
      rethrow;
    }
  }
  
  // Helper method to generate email with specific intent
  Future<String> generateEmailWithIntent({
    required String emailContent,
    required String subject,
    required String sender,
    required String receiver,
    required String intent,
    String language = 'vietnamese',
  }) async {
    String mainIdea = '';
    String action = '';
    
    switch (intent) {
      case 'thanks':
        mainIdea = 'Cảm ơn về thông tin đã cung cấp.';
        action = 'Viết email cảm ơn';
        break;
      case 'sorry':
        mainIdea = 'Xin lỗi về sự bất tiện đã gây ra.';
        action = 'Viết email xin lỗi';
        break;
      case 'yes':
        mainIdea = 'Đồng ý với đề xuất/yêu cầu.';
        action = 'Viết email đồng ý';
        break;
      case 'no':
        mainIdea = 'Từ chối đề xuất/yêu cầu một cách lịch sự.';
        action = 'Viết email từ chối lịch sự';
        break;
      case 'followup':
        mainIdea = 'Theo dõi về vấn đề đã đề cập trước đó.';
        action = 'Viết email theo dõi';
        break;
      case 'moreinfo':
        mainIdea = 'Yêu cầu thêm thông tin về vấn đề.';
        action = 'Viết email yêu cầu thêm thông tin';
        break;
      default:
        mainIdea = 'Phản hồi email.';
        action = 'Viết email phản hồi';
    }
    
    return generateEmailResponse(
      emailContent: emailContent,
      mainIdea: mainIdea,
      action: action,
      subject: subject,
      sender: sender,
      receiver: receiver,
      language: language,
    );
  }
  
  void clearError() {
    _error = '';
    notifyListeners();
  }
}
